...
- **モード切替**: F1（テスト）/ F2（バグ調査）/ F3（セットアップ）で時間配分を自動計測（TBS メトリクス）。モード行に T / B / S の比率を示す小さなバーが常時表示される
- **スクショ**: F9 で現在の画面をキャプチャ。注釈（矩形・矢印・モザイク・テキスト）を即座に追加可能。画面共有が非アクティブな場合やセッション一時停止中はトースト通知で知らせる
- **タイムライン**: すべてのアクションが `MM:SS [TAG] 本文` の形式で記録される
- **バグ検出時**: テスト中に BUG エントリを記録すると、F2 キーでバグ調査モードに切り替えるよう促すヒントが表示される
